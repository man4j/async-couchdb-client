...
public class CouchDbResponseHandler<F, T> {
    ...
    public CouchDbResponseHandler(HttpClientProviderResponse response, TypeReference<F> typeReference, Function<F, T> transformer, ObjectMapper mapper, OperationInfo opInfo, CouchDbOperationStats couchDbOperationStats) {
        this(response, TypeFactory.defaultInstance().constructType(typeReference), transformer, mapper, opInfo, couchDbOperationStats);
    }

    public CouchDbResponseHandler(HttpClientProviderResponse response, JavaType javaType, Function<F, T> transformer, ObjectMapper mapper, OperationInfo opInfo, CouchDbOperationStats couchDbOperationStats) {
    ...
    public T transform() {
        try {
            ...
            couchDbHttpResponse = new CouchDbHttpResponse(statusCode, "", body, response.getUri().toString());
