...
public class OperationProfile {
    ...
    public void incCount() {
        count.labels(database, operationType.toString(), operationInfo, stackTrace).inc();
        _count++;
    }
    
    public void setAvgOperationTime(long amount) {
        avgOperationTime.labels(database, operationType.toString(), operationInfo, stackTrace).set(amount);
    }
    ...
    public void setMaxOperationTime(long amount) {
        maxOperationTime.labels(database, operationType.toString(), operationInfo, stackTrace).set(amount);
    ...
    public void setMinOperationTime(long amount) {
        minOperationTime.labels(database, operationType.toString(), operationInfo, stackTrace).set(amount);
        _minOperationTime = amount;
    }
    
    public void addTotalTime(long amount) {
        totalTime.labels(database, operationType.toString(), operationInfo, stackTrace).inc(amount);
        _totalTime += amount;
    }
    
    public void addSize(long amount) {
        size.labels(database, operationType.toString(), operationInfo, stackTrace).inc(amount);
    }
